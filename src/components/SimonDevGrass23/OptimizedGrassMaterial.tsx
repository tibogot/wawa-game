...
interface MaterialConfig {
  ...
  sssScale?: number;
  sssColor?: string;
  // Environment Map/IBL controls (v23 addition)
  ...
  const updateUniforms = useCallback(() => {
    ...
    uniformUpdateQueue.current.forEach((material) => {
      if (material.userData.shader) {
        ...
        // Update live-config uniforms (v23 extras)
    ...
    newMaterial.onBeforeCompile = (shader) => {
      ...
      shader.fragmentShader = shader.fragmentShader.replace(
        ...
        `
          ...
          // Environment Map reflections (v23) - removed, causes shader errors
      ...
      shader.fragmentShader = shader.fragmentShader.replace(
        ...
        // Subsurface Scattering - Enhanced for better visibility (v23)
        ...
        // Environment Map reflections (v23) - after lighting where geometryNormal/viewDir exist
      ...
      if (!(newMaterial as any)._loggedFragment) {
        ...
        try {
          console.log("[Grass23] Vertex shader:\n", shader.vertexShader);
          console.log("[Grass23] Fragment shader:\n", shader.fragmentShader);
        } catch {}
      }
    };
