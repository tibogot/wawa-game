...
void main() {
  ...
  // Mix normals for more 3D appearance - using vNormal (from Three.js) and vNormal2 (custom)
  ...
  // Custom fog calculation - more realistic atmospheric effects
  vec3 viewDir = normalize(cameraPosition - vWorldPosition);
  float fogDepth = vFogDepth * vFogDepth;
  float fogScatterDensity = 0.0005;
  float fogExtinctionDensity = 0.003;
  
  // Simple sky color for fog (can be enhanced later)
  vec3 fogSkyColor = vec3(0.39, 0.57, 0.86) * 0.25;
  
  // Calculate fog factors
  float fogScatterFactor = exp(-fogScatterDensity * fogScatterDensity * fogDepth);
  float fogExtinctionFactor = exp(-fogExtinctionDensity * fogExtinctionDensity * fogDepth);
  
  // Apply fog to outgoingLight
  outgoingLight = outgoingLight * fogExtinctionFactor + fogSkyColor * (1.0 - fogScatterFactor);
