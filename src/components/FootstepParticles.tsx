...
export const FootstepParticles = forwardRef<FootstepParticlesHandle>((_, ref) => {
  ...
  useImperativeHandle(
    ...
    () => ({
      spawn: ({ position, normal }) => {
        ...
        const up = normal && normal.lengthSq() > 0 ? normal : undefined;
        ...
        if (up) {
          velocity.addScaledVector(up, 0.6);
        } else {
          velocity.y += 0.4;
        }
